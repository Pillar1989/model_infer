...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        
        {
            ...
            // 11. 可视化结果
            if (show_result) {
                // Reload image for drawing (original image might be modified or we want a clean copy)
                // Actually, we can use the original 'img' object if it wasn't modified in place destructively
                // But preprocess modifies 'img' (resize, pad).
                // So we should reload the original image.
